...
//! # ) -> Result<()> {
//! // For each argument, apply its validation rules
...
//!                         file_validator::validate_file_exists(path, &arg_def.name)?;
//!                     }
//!                 }
...
//!                         .map_err(|_| ValidationError::CustomConstraint {
//!                             arg_name: arg_def.name.clone(),
//!                             reason: "Cannot parse as number".to_string(),
//!                         })?;
//!                     
//!                     range_validator::validate_range(
//!                         num_value,
//!                         &arg_def.name,
//!                         *min,
//!                         *max
...
mod tests {
    use super::*;
    use crate::config::schema::{ValidationRule, ArgumentType, ArgumentDefinition};
    use std::path::Path;
    use std::fs;
    use std::io::Write;
    ...
    fn test_validate_parsed_arguments() {
        ...
        parsed_args.insert("input".to_string(), input_path.to_str().unwrap().to_string());
