...
- Gouverne le chaînage de commandes (v0.8.0, [DD-026](https://github.com/biface/dcli/issues/52)) — sans effet quand cette commande n'appartient pas à une chaîne.
...
- Fait aussi partie du chaînage de commandes ([DD-026](https://github.com/biface/dcli/issues/52)).
...
distincte à activer. `continue_on_failure` et `requires_success`,
décrits ci-dessus, gouvernent le comportement de chaque commande au
sein d'une chaîne.
...
#### Contrôle de cohérence côté handler

*Depuis la v0.9.0 ([DD-028](https://github.com/biface/dcli/issues/64)).*

`continue_on_failure` est fixé en YAML, indépendamment du code du
handler — rien ne relie les deux par défaut, donc un handler écrit en
supposant que ses échecs arrêtent toujours la chaîne pourrait se
retrouver configuré silencieusement avec `continue_on_failure: true`
(ou l'inverse). Il n'y a aucun nouveau champ YAML pour cela : c'est un
contrôle de cohérence côté Rust, posé sur le champ existant, vérifié
une seule fois, à l'enregistrement, avant même que l'application ne
s'exécute.

L'auteur d'un handler peut déclarer une attente en surchargeant
`expected_fault_tolerance()` sur `CommandHandler` ou
`AsyncCommandHandler` (les deux valent `None` par défaut — aucune
opinion, aucun contrôle) :
...
impl CommandHandler for SolveCommand {
    ...
    fn expected_fault_tolerance(&self) -> Option<bool> {
        Some(false) // ce handler suppose qu'il arrête toujours la chaîne
    }
}
```

`CommandRegistry::register_sync`/`register_async` comparent cette
valeur (quand elle vaut `Some`) au `continue_on_failure` réellement
configuré pour la commande, et rejettent l'enregistrement avec
`RegistryError::FaultToleranceMismatch` en cas de contradiction —
détectant une mauvaise configuration YAML au démarrage plutôt qu'au
premier usage dans une chaîne. Un handler qui ne surcharge jamais la
méthode (le défaut, `None`) n'est absolument pas concerné, quelle que
soit la valeur de `continue_on_failure`.

Ceci ne vérifie **pas** la logique interne du handler — que son
`execute()` retourne bien `Err(...)` quand il le devrait. Cela reste
hors de portée de toute vérification au niveau de la signature,
couvert par les tests propres de l'application.
